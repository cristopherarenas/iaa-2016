...
void Inmune::generar_poblacion(SolutionSet &poblacion)
{
	...
	{
		cout << "cantidad " << cantidad << ", intentos " << intentos << endl;
...
void Inmune::afinidad(vector<Solution> &poblacion){
	for(int i=0;i<poblacion.size();i++)
	{
		poblacion[i].setQuality(this->opciones.get_alpha(),this->opciones.get_beta());
	}
}
...
void Inmune::eliminar_exceso(vector<Solution> &clones)
{
	...
	sort(afinidades.begin(),afinidades.end());
	/*
	cout << "afinidades size " << afinidades.size() << endl;
	for(int i=0;i<afinidades.size();i++){
		cout << afinidades[i] << endl;
	}
	*/
	clones.clear();
	...
	*/
	...
	cout << afinidades.size() << " " << clones.size() << endl;
