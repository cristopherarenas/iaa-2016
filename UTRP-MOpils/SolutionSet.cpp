...
SolutionSet::SolutionSet(Problem &p, vector<RouteInfo> &ri,int &pop_size,ShortestRoute &sro,float &alpha, float &beta) 
{
	vector<Solution> sol;
	this->solutions=sol;
	bool sol_conexa = false;
	...
	for(int i=0;i<pop_size;i++){
		do{
			//cout << "intentando generar solucion";
			Solution *s = new Solution(p,ri,sro,alpha,beta);
