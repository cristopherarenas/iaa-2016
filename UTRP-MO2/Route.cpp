...
Route::Route(Problem &p, RouteInfo &ri){
	this->tipo_ruta = ri.tipo_ruta;	
	vector<BusStop>::iterator it;
	//random length of the route
	int longitud = ri.min_length+rand()%(ri.max_length-ri.min_length);
	//random select of the first busstop 
	int random_bs = rand()%(p.get_bus_stops().size());
	//reserve space for bus stops
	this->bus_stops.reserve(ri.max_length);
	//asign for route
	this->bus_stops.push_back(p.get_bus_stops()[random_bs]);
	//find two neighbors
	int lroute = random_bs;
	int rroute = random_bs;
	bool flag = true;
	
	for(unsigned int k=0;k<(longitud/2);k++){
		//find a left and right neighbor for the route
		for(unsigned int i = 0; i< p.get_bus_stops()[rroute].neighbors.size();i++){
			
			random_bs = rand()%(p.get_bus_stops()[rroute].neighbors.size());
			
			if(!find_node(p.get_bus_stops()[rroute].neighbors[random_bs]))
			{
				this->bus_stops.push_back(p.get_bus_stops()[p.get_bus_stops()[rroute].neighbors[random_bs]]);
				rroute = p.get_bus_stops()[rroute].neighbors[random_bs];
				break;
			}
		}
		
		for(unsigned int i = 0; i< p.get_bus_stops()[lroute].neighbors.size();i++){
			
			random_bs = rand()%(p.get_bus_stops()[lroute].neighbors.size());
			
			if(!find_node(p.get_bus_stops()[lroute].neighbors[random_bs]))
			{
				it = this->bus_stops.begin();
				this->bus_stops.insert(it,p.get_bus_stops()[p.get_bus_stops()[lroute].neighbors[random_bs]]);
				lroute = p.get_bus_stops()[lroute].neighbors[random_bs];
				break;
			}
		}
	}
