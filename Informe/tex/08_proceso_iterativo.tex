...
%Donde $\tilde{FO}_1$ y $\tilde{FO}_2$ corresponden a las funciones objetivo dadas por las ecuaciones \eqref{eq:fo1} y \eqref{eq:fo2}, respectivamente. $\alpha$ y $\beta$ son parámetros del algoritmo y ponderan la importancia que se le da a ambos objetivos. Se considera como restricción de estos parámetros que $\alpha+\beta=1$.
...
\begin{enumerate}
\item \textbf{Agregar una ruta}: Considerando una ruta $r$ con $n$ paraderos ordenados: $bs_1, bs_2, \ldots, bs_n$. Este operador busca un vecino del paradero $bs_n$ y le agrega un nuevo paradero $bs_{n+1}$ al final de la ruta, dejando la ruta con $n+1$ paraderos ordenados. La Figura \ref{fig:mut1} muestra un ejemplo con $n=6$.
...
\item \textbf{Eliminar una ruta}: Considerando una ruta $r$ con $n$ paraderos ordenados: $bs_1, bs_2, \ldots, bs_n$. Este operador elimina el último paradero $bs_n$, dejando la ruta con $n-1$ paraderos ordenados. La Figura \ref{fig:mut2} muestra un ejemplo con $n=6$.
