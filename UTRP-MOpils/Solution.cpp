...
Solution::Solution(Problem &p, vector<RouteInfo> &routes_info)
{
	...
	for(int i=0;i<routes_info.size();i++){
		for(int j=0;j<routes_info[i].quantity;j++){
			...
			Route *r = new Route(p,routes_info[i]);
			//cout << j << " ";
...
Solution::Solution(Problem &p, vector<RouteInfo> &routes_info,ShortestRoute &sroute, float &alpha, float &beta)
{
	//crear nueva solucion factible
	this->dominated = false;
	this->ranking_pareto = 1;
	...
	int size  = p.get_size();
	float lb_fo1 = p.get_lbfo1();
	float lb_fo2 = p.get_lbfo2();
	
	this->setFO1(sroute,demand);
	this->setFO2(size,travel);
	this->setQuality(alpha,beta,lb_fo1,lb_fo2);
}
...
void Solution::setQuality(float alpha, float beta,float lbfo1,float lbfo2)
{
	float aptitud;
	aptitud = alpha*(this->fo1/lbfo1)+beta*(this->fo2/lbfo2);
	this->quality=aptitud;
...
bool Solution::mutation(vector<RouteInfo> &info, int** &travel,int &size,vector<BusStop> &bs){
	...
	Route r = this->routes[route_selected];
	int intentos = 0;
	//cout << "route size " << this->routes.size() << endl;

	do{
		//cout << "route_selected " << route_selected << endl;
		//cout << "antes ";
		//this->routes[route_selected].print_route();
		if(this->routes[route_selected].bus_stops.size()>=info[this->routes[route_selected].tipo_ruta].max_length){
			...
			//cout << "case 1: op " << op_mutacion << endl;
			if(op_mutacion == 0){
			...
			int op_mutacion = (100*rand())%2;
			//cout << "case 2: op " << op_mutacion << endl;
			if(op_mutacion == 0){
			...
			//cout << "case 3: op " << op_mutacion << endl;
			if(op_mutacion == 0){
				change = mutation_remove_bs(this->routes[route_selected],size);
			}
			else if(op_mutacion == 1){
